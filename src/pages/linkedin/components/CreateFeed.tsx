    ...
    <div className="bg-white sm:rounded-lg p-4 shadow-bg">
      ...
      <div className="flex items-center justify-evenly">
        {FEED_ACTIONS.map((action) => (
          <button
            key={action.label}
            className="flex items-center justify-center gap-1 cursor-pointer px-4 py-2 text-base leading-2xl font-[600] text-[#000000bf] hover:bg-[#0000000d] transition-all duration-300 rounded-md flex-1"
          >
            {action.icon}
            <p className="text-sm font-[600] text-nowrap">{action.label}</p>
          </button>
        ))}
      </div>
    </div>
